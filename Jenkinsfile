    ...
    stage('CodeCoverage') {
      steps {
        container('maven') {
          ...
          jacoco(execPattern: 'target/jacoco.exec', classPattern: 'target/classes', sourcePattern: 'src/main/java', exclusionPattern: 'src/test*', changeBuildStatus: true, minimumLineCoverage: '80' )
        }
      }
    }
